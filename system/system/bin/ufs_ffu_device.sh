...
action=`getprop ro.boot.ffu_action`
if [ "$action" != "" ]; then
	kmsg_print "will  execute  $action directly without ufs ffu!"
	sleep 1
	if [ "$action" == "reboot" ]; then
		kmsg_print " directly reboot"
		echo b >/proc/sysrq-trigger
	elif [ "$action" == "panic" ]; then
		kmsg_print " directly panic"
		echo c >/proc/sysrq-trigger
	elif [ "$action" == "poweroff" ]; then
		kmsg_print " directly pweroff"
		echo o >/proc/sysrq-trigger
	elif [ "$action" == "skip_ffu" ]; then
		kmsg_print " skip ufs ffu, ufs_ffu_device.sh exit"
		exit 0
	fi
fi
if [ -f $CONFIG_FN ]; then
...
if [ "$VENDOR" == "SAMSUNG" -o "$VENDOR" == "SKhynix"  ]; then
	if [ -x /system_ext/bin/sg_write_buffer ] ; then
		SG_WRITE_BUFFER_TOOL=/system_ext/bin/sg_write_buffer
		kmsg_print "$VENDOR firmware upgrade tool supported."
	elif [ -x /system_ext/bin/sg_write_buffer_msi ]; then
		SG_WRITE_BUFFER_TOOL=/system_ext/bin/sg_write_buffer_msi
		kmsg_print "$VENDOR firmware upgrade tool supported."
	else
...
if [ "$VENDOR" == "MICRON" ] ; then
	...
	$SG_WRITE_BUFFER_TOOL -v -m dmc_offs_defer -b 524288 -I $FW_FILE  /dev/block/sda > /dev/kmsg 2>&1
else
