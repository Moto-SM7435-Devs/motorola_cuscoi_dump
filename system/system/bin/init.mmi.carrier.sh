...
is_fcnt=`getprop ro.product.is_fcnt`
    ...
    case $carrier in
        ...
        tefes|o2gb|o2de )
            ...
            if [ "$gms_premier_tier" == true ] ; then
                ...
                setprop ro.mot.gms.clientidbase.vs android-tef-{country}-rvc2
            fi
        ;;
        ...
        # Docomo Japan
        docomo )
            setprop ro.mot.gms.clientidbase.am android-docomo-jp
        ;;

        # KDDI Japan
        kddi )
            setprop ro.mot.gms.clientidbase.am android-kddi-jp
            if [ "$is_fcnt" == true ] ; then
                setprop ro.mot.gms.clientidbase.ms android-kddi-jp-rvc2
            else
                setprop ro.mot.gms.clientidbase.ms android-kddi-jp-rvc3
            fi
        ;;
        ...
        rettr )
            setprop ro.mot.gms.clientidbase.ms android-motorola
            setprop ro.mot.gms.clientidbase.vs android-motorola
        ;;
        ...
        * )
            ...
            if [ "$is_fcnt" == true ]; then
                setprop ro.mot.gms.clientidbase.ms ""
                setprop ro.mot.gms.clientidbase.vs ""
            else
                if [ "$gms_premier_tier" == true ]; then
                    setprop ro.mot.gms.clientidbase.ms android-motorola-rvo3
                else
...
set_carrier_readonly_properties ()
{
    case $boot_carrier in
        3gb )
            setprop ro.csc.amazon.partnerid 3uk
        ;;
        amxmx )
            setprop ro.csc.amazon.partnerid telcelmx
        ;;
        att )
        ...
        ;;
        docomo )
            setprop ro.csc.amazon.partnerid docomo
        ;;
        tmo )
...
set_carrier_media_dir_property ()
{
    ...
    if [ ! $propMediaDir ]; then
        ...
        colorProp=`getprop ro.vendor.hw.frontcolor`
        carrier_color_folder="/product/carrier/${propChannelId}_color_${colorProp}/media"
        # <carrier>_color_<color> is used if exists
        if [ -d "$carrier_color_folder" ]; then
            setprop persist.carrier.media.dir "$carrier_color_folder"
        else
